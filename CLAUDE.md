...
High-performance Rust MCP server for semantic code indexing. Implements a walker → splitter → embedder → Milvus pipeline for codebase search via Claude Code.
...
| `src/mcp/indexer.rs` | 3-phase indexing pipeline |
| `src/mcp/searcher.rs` | Semantic search implementation |
| `src/mcp/tools.rs` | MCP tool definitions and JSON schemas |

## MCP Tools

1. **index_codebase** - Walk, split, embed, and store a directory
2. **search_code** - Semantic search over indexed codebase
3. **get_indexing_status** - Check indexing progress
4. **clear_index** - Remove indexed data
...
**Tested modules:**
- `vectordb/mod.rs` - Collection name generation
- `splitter/parsers.rs` - Parser availability
- `splitter/node_types.rs` - Splittable node types
- `splitter/extractor.rs` - Chunk extraction
- `config.rs` - Default configuration
- `embedding/mod.rs` - Embedding client
